...
function ETDRK4(SLPDE::SemiLinearPDE{2, S}, T::Number, n::Int) where S
    ...
    P = plan_sph2fourier(U[1])
    PS = plan_sph_synthesis(U[1])
    PA = plan_sph_analysis(U[1])
    ...
    Z = L[1]*h, L[2]*h
    ez = sph_zero_spurious_modes!(exp.(Z[1])), sph_zero_spurious_modes!(exp.(Z[2]))
    ez2 = sph_zero_spurious_modes!(exp.(Z[1]./2)), sph_zero_spurious_modes!(exp.(Z[2]./2))
    h2ez2m1 = sph_zero_spurious_modes!((h/2.0).*φ1.(Z[1]./2)), sph_zero_spurious_modes!((h/2.0).*φ1.(Z[2]./2))
    hezα = sph_zero_spurious_modes!(h.*expα.(Z[1])), sph_zero_spurious_modes!(h.*expα.(Z[2]))
    h2ezβ = sph_zero_spurious_modes!(2h.*expβ.(Z[1])), sph_zero_spurious_modes!(2h.*expβ.(Z[2]))
    hezγ = sph_zero_spurious_modes!(h.*expγ.(Z[1])), sph_zero_spurious_modes!(h.*expγ.(Z[2]))
    ...
    for k=1:n
        ...
        NU[1] .= U[1]
        NU[2] .= U[2]
        evaluate_nonlinear_operator!(SLPDE.N, P, PS, PA, NU, TEMP)
        A[1] .= ez2u[1] .+ h2ez2m1[1].*NU[1]
        A[2] .= ez2u[2] .+ h2ez2m1[2].*NU[2]
        NA[1] .= A[1]
        NA[2] .= A[2]
        evaluate_nonlinear_operator!(SLPDE.N, P, PS, PA, NA, TEMP)
        B[1] .= ez2u[1] .+ h2ez2m1[1].*NA[1]
        B[2] .= ez2u[2] .+ h2ez2m1[2].*NA[2]
        NB[1] .= B[1]
        NB[2] .= B[2]
        evaluate_nonlinear_operator!(SLPDE.N, P, PS, PA, NB, TEMP)
        C[1] .= ez2[1].*A[1] .+ h2ez2m1[1].*(2.0.*NB[1] .- NU[1])
        C[2] .= ez2[2].*A[2] .+ h2ez2m1[2].*(2.0.*NB[2] .- NU[2])
        NC[1] .= C[1]
        NC[2] .= C[2]
        evaluate_nonlinear_operator!(SLPDE.N, P, PS, PA, NC, TEMP)
...
function ETDRK4(SLPDE::SemiLinearPDE{1, S}, T::Number, n::Int, surf, io; colorrange::Symbol=:fixed) where S
    U0 = SLPDE.U0[1]
    U = deepcopy(U0) # solution spherical harmonic coefficients
    NU = zero(U) # nonlinearity spherical harmonic coefficients
...
function ETDRK4(SLPDE::SemiLinearPDE{2, S}, T::Number, n::Int, surf, io; colorrange::Symbol=:fixed) where S
    U0 = SLPDE.U0
    U = deepcopy(U0) # solution spherical harmonic coefficients
    NU = zero(U[1]), zero(U[2]) # nonlinearity spherical harmonic coefficients
    TEMP = deepcopy(NU[1])
    F = zero(U[1]) # solution function values
    Ft = zeros(Float32, size(F, 1)+2, size(F, 2)+1)
